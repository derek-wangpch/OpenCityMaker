...
- **Preferences** (the slider icon in the header, a `#/settings` page on a phone) gathers every option: move style, tile numbers, weather, appearance, language and reduced motion.
- **One-cell moves** is a Threes!-style ruleset: a swipe nudges the board a single cell instead of sliding it to the wall. Scanning from the edge you swiped toward, the first tile that can enter the cell ahead — a gap, or an equal value — does so, and the tiles behind it follow by one cell; a line still merges at most once per move. Switching the move style mid-run asks to start a new city, so a score is always earned under one ruleset. The choice is remembered per browser.
...
- On an iPhone or iPad, **Add to Home Screen** on the phone Start screen walks through the browser share sheet. CityMaker then launches full screen in portrait with its own icon, reading the same saves as the browser tab.
- A weather button in the board toolbar cycles clear → cloudy → rain → snow → fog → off, and Preferences carries a single weather switch for turning it on or off. Weather changes automatically every 45–90 seconds unless off, so a state picked in the toolbar is a mood for the next minute rather than a saved preference; web transitions blend over three seconds. Weather dims or brightens the scene lighting, tints the CSS sky to match, and stays remembered per browser. Reduce motion freezes weather on a static frame; the model viewer and thumbnails never show weather.

Weather choices follow typical urban conditions: Hong Kong, Shenzhen, Singapore, Dubai and Sydney skip snow in both the toolbar and automatic changes. Snow saved for, or carried into, these cities resets to clear; other weather choices, including off, are preserved. This is a gameplay choice, not a claim that rare historical or nearby mountain snowfall is impossible.
...
The save payload remains version 1 with additive session metadata, including the optional `showLabels`, `weather` (`clear`/`cloudy`/`rain`/`snow`/`fog`/`off`; absent means clear) and `moveMode` (`slide`/`step`; absent means slide) preferences. Invalid city saves are discarded independently; unavailable IndexedDB allows temporary play with a visible notice. Saves do not synchronize between devices. The initial font request uses Google Fonts; local system fonts are the fallback.

Home screen support is the Apple route only, because Safari exposes no install API: `src/homeScreen.ts` recognizes an iPhone or iPad and the phone Start screen offers the share-sheet steps. The button hides itself once the page already runs standalone, and in embedded webviews (WeChat and other in-app browsers) whose share sheet has no Home Screen entry. Chromium's `beforeinstallprompt` is deliberately unused: without a service worker it would not fire. The launch appearance comes from `public/manifest.webmanifest`, the `apple-touch-icon` PNG, and the Apple meta tags in `index.html`; `public/favicon.svg` is the single source of the mark, matching the icon on the production site, and `npm run build:icons` rasterizes the PNGs from it with Playwright's Chromium. The rasterizer paints the mark's own plate color edge to edge, because iOS composites transparent pixels onto black and applies its own rounded mask. The results are versioned so a normal build never needs a browser. There is still no service worker, so an installed CityMaker needs a connection to open and guaranteed offline caching remains outside this release.
...
AI-assisted model contributors can invoke `$building-reference-modeling` from the repository's [reference-based building modeling skill](.agents/skills/building-reference-modeling/SKILL.md). It guides multi-view research, tier-appropriate detail, stylized geometry, and visual validation while keeping uncertain inferences explicit.
