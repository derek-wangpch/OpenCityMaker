...
- 棋盘下方的两个旋转按钮每次顺时针或逆时针转动 45°，连续八次回到初始视角。沿当前画面的棋盘方向滑动，键盘和方向按钮也会随视角对应。
- 游玩和合并动画中都可旋转；旋转不移动方块、不生成建筑、不计步或改变得分。刷新和切换城市后会记住所选角度。
- 两座相同建筑相遇时会合并为下一等级，并把合并后的数值加入分数。
- 达到 2048 即完成当前城市；棋盘填满且无法继续合并时结束。
- 桌面和手机棋盘工具栏的「撤销」按钮可撤回上一次有效移动，恢复棋盘、分数和步数，胜利或失败后也可使用。每步只能撤销一次，继续移动后可再次撤销；新开局时不可用。撤销和重新开始都不会清除图鉴和最高分。
- 切换城市会保留各自独立的棋盘和进度。
- 天气按常见城区情况设置：香港、深圳、新加坡、迪拜和悉尼在手动与自动切换时均跳过下雪。旧存档或切换城市带来的雪天会改为晴天，其他天气及关闭状态保留；此规则不代表历史极端天气或附近山区绝不会下雪。
...
仓库中的 132 个微缩建筑均以程序化几何原创实现。[docs/CREATION.md](docs/CREATION.md) 记录了这些模型的制作方式，包括使用 GPT-6 Astra 生成模型几何，以及记录中哪些内容可以对照源码核对。建筑比例会为了棋盘可读性进行压缩和风格化，并不用于测绘或历史复原。建筑资料中的来源链接用于记录视觉研究依据，不代表对第三方照片、商标或建筑作品拥有权利。
