...
for (const theme of ["light", "dark"] as const) {
  test(`Dubai courtyard hierarchy in ${theme} mode`, async ({ page }) => {
    ...
    const capture = async ({ path }: { path: string }) => {
      ...
      await mkdir(`artifacts/dubai-saeed-redesign/${theme}`, {
        recursive: true,
      });
        ...
        JSON.stringify({
          ...
          cities: {
            dubai: {
              run: {
                board: [
                  64, 16, 32, 4, 16, 64, 4, 32, 32, 4, 64, 16, 4, 32, 16, 64,
                ],
                score: 4096,
                status: "playing",
                undo: null,
    ...
    await capture({
      path: `artifacts/dubai-saeed-redesign/${theme}/mobile-board.png`,
    });
    ...
    for (const tier of [6, 5, 4]) {
      ...
      await capture({
        path: `artifacts/dubai-saeed-redesign/${theme}/mobile-tier-${tier}.png`,
      });
      ...
      await page.mouse.up();
      await capture({
        path: `artifacts/dubai-saeed-redesign/${theme}/rotated-tier-${tier}.png`,
      });
